...
def make_map(selected_year):
    ...
    fig = px.choropleth(year_df, locations="Code", color="CO2_category", hover_name="Entity", custom_data=["CO2_label", "Year", "Code"], category_orders={"CO2_category": labels}, color_discrete_map=color_map, title=f"Global CO₂ Emissions & Climate Indicators in {selected_year}")
    
    # Hide Categorical CO2 Legend
    fig.update_traces(hovertemplate="<b>%{hovertext}</b><br>Total CO2: %{customdata[0]}<br>Year: %{customdata[1]}<extra></extra>", showlegend=False)

    ice_disp = {"No data": "No Data Recorded", "< 3": "< 3M km²", "3-10": "3M–10M km²", "> 10": "> 10M km²"}
    for i, cat in enumerate(ice_order):
        fig.add_trace(go.Scattergeo(lon=[None], lat=[None], mode="markers", marker=dict(size=10, color=ice_color_map[cat]), name=ice_disp[cat], legendgroup="ICE", legendgrouptitle_text="<b>SEA ICE EXTENT</b>" if i == 0 else None, showlegend=True, hoverinfo="skip"))
    ...
    for i, cat in enumerate(sea_level_order):
        fig.add_trace(go.Scattergeo(lon=[None], lat=[None], mode="markers", marker=dict(size=10, color=sea_level_color_map[cat]), name=sea_disp[cat], legendgroup="LEVEL", legendgrouptitle_text="<b>SEA LEVEL CHANGE</b>" if i == 0 else None, showlegend=True, hoverinfo="skip"))

    ice_pts = get_sea_ice_points(selected_year)
    fig.add_trace(go.Scattergeo(lon=ice_pts["lon"], lat=ice_pts["lat"], mode="markers", marker=dict(size=ice_pts["base_size"], color="rgba(128,128,128,0.3)", line=dict(color="black", width=1.5, dash="dot")), showlegend=False, hoverinfo="skip"))

    for cat in ice_order:
        sub = ice_pts[ice_pts["ice_cat"] == cat]
        if not sub.empty:
            fig.add_trace(go.Scattergeo(lon=sub["lon"], lat=sub["lat"], mode="markers+text", text=sub["inside_text"], textposition="middle center", customdata=sub[["Region", "value_label", "pct_text", "series_type"]].values, marker=dict(size=sub["size"], color=ice_color_map[cat], line=dict(color="navy", width=1)), textfont=dict(color="black", size=10), showlegend=False, hovertemplate="<b>%{customdata[0]}</b><br>Peak extent: %{customdata[1]}<br>Change vs 1979: %{customdata[2]}<extra></extra>"))

    sea_pt = get_sea_level_point(selected_year)
    fig.add_trace(go.Scattergeo(lon=sea_pt["lon"], lat=sea_pt["lat"], mode="markers+text", text=sea_pt["inside_text"], textposition="middle center", customdata=sea_pt[["Name", "value_label", "pct_text", "series_type"]].values, marker=dict(size=sea_pt["size"], symbol="square", color=[sea_level_color_map[c] for c in sea_pt["sea_cat"]], line=dict(color="navy", width=2)), textfont=dict(color="black", size=12), showlegend=False, hovertemplate="<b>%{customdata[0]}</b><br>Level: %{customdata[1]}<br>%{customdata[2]}<extra></extra>"))
    ...
    fig.add_trace(go.Scattergeo(lon=[-163], lat=[-56], mode="markers", marker=dict(size=14, color="rgba(0,0,0,0)"), customdata=[["Temperature", "special"]], showlegend=False, hovertemplate=f"<b>Temperature</b><br>Year: {selected_year}<br>Anomaly: {tv:+.2f}°C<extra></extra>"))

    fig.update_geos(showcountries=True, countrycolor="black", showocean=True, oceancolor="#d6f2ff", lataxis_range=[-90, 90], lonaxis_range=[-180, 180], projection_type="equirectangular")
    fig.update_layout(margin=dict(l=10, r=10, t=180, b=10), height=750, legend=dict(yanchor="top", y=0.87, xanchor="left", x=1.01, bgcolor="rgba(255, 255, 255, 0.9)", bordercolor="#444444", borderwidth=1, traceorder="grouped", itemsizing="constant", font=dict(size=10)))
    return fig
...
def make_sea_ice_bar(region, selected_year):
    ...
    if selected_year < 1979:
        ...
        fig.update_layout(xaxis_visible=False, yaxis_visible=False, height=440); return fig
    row = df[df["Year"] == selected_year]
    if row.empty: return make_empty_detail(selected_year)
    ...
    fig = go.Figure([
        go.Bar(x=[vals[0]], y=[labs[0]], orientation="h", marker_color="indianred", text=[f"{vals[0]:.2f} million km²"], textposition="outside", cliponaxis=False, name="Min"),
        go.Bar(x=[vals[1]], y=[labs[1]], orientation="h", marker_color="steelblue", text=[f"{vals[1]:.2f} million km²"], textposition="outside", cliponaxis=False, name="Max")
    ])
    fig.update_layout(title=f"{region} sea ice extent in {selected_year}", xaxis_title="Sea ice extent (million km²)", height=440, showlegend=False, xaxis_range=[0, max(vals)*1.35], margin=dict(r=120))
    return fig
...
def make_source_area(country_code, selected_year):
    ...
    fig = px.area(df, x="Year", y="Value", color="Source", title=f"CO2 Source Distribution: {df['Entity'].iloc[0] if not df.empty else ''}")
    fig.add_vline(x=selected_year, line_dash="dash", line_color="black")
...
def make_pie(country_code, selected_year):
    ...
    fig = px.pie(df[df["Value"] > 0], names="Source", values="Value", title=f"CO2 sources: {row['Entity'].iloc[0]} ({selected_year})", hole=0.35)
    fig.update_traces(textinfo="percent+label", hovertemplate="<b>%{label}</b><br>%{value:,.0f} tonnes<extra></extra>")
    fig.update_layout(margin=dict(l=20, r=20, t=55, b=20), height=320)
    return fig
...
def make_temperature_line(selected_year):
    ...
    fig.add_vline(x=selected_year, line_dash="dash", line_color="black")
    fig.update_layout(title="Global temperature anomaly over time", xaxis_title="Year", yaxis_title="Anomaly (°C)", height=320)
    return fig
...
def make_sea_ice_area(region, selected_year):
    ...
    fig.add_vline(x=selected_year, line_dash="dash", line_color="black")
    fig.update_layout(title=f"{region} sea ice extent over time", yaxis_title="M km²", height=420)
    return fig
